...
| `OPENCLAW_COMPACTION_RESERVE_TOKENS_FLOOR` | Minimum `agents.defaults.compaction.reserveTokensFloor` merged on each start. **Default depends on `INJECTED_OR_MODEL`:** `openrouter/auto` (Advanced) uses a **high** floor; `openrouter/auto:free,*-small*…` (Medium) and `openrouter/free` use a **low** floor and a **cap** (~8k / ~6k) so **~16k-context** models are not starved (a 98k floor on a 16k window causes immediate overflow). ClawDeez provision sets this per tier. Set to `0` or `skip` to skip. |
