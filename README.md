...
| `OPENCLAW_COMPACTION_RESERVE_TOKENS_FLOOR` | Minimum `agents.defaults.compaction.reserveTokensFloor` merged on each start (default **48000**; raises older 20k caps). Reduces **“Context limit exceeded”** in Control UI chat. Set to `0` or `skip` to skip. |
