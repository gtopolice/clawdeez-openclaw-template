...
| `OPENCLAW_COMPACTION_RESERVE_TOKENS_FLOOR` | Minimum `agents.defaults.compaction.reserveTokensFloor` merged on each start (default **98304**). Reduces **“Context limit exceeded”** in Control UI chat when workspace bootstrap (AGENTS.md, etc.) is large—**no need to change model tier**. Set to `0` or `skip` to skip. |
